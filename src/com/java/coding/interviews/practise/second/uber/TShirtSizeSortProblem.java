 ...
 * if we use counting sort, it will take O(n)  + O(n) time and O(n) space.
 */
public class TShirtSizeSortProblem {
    public static void main(String[] args) {
        char[] sizes = {'M', 'S', 'L', 'M', 'S', 'L', 'M', 'S'};
        //sortTShirtSize(sizes);
        sortTShirtSizeUsingCountingSort(sizes);
        System.out.println(Arrays.toString(sizes));
    }

    //Customize sorting for T-shirt sizes
    //Time Complexity: O(n)
    ...
    public static void sortTShirtSize(char[] size){
        //Arrays.sort(size); // O(n log n) time complexity
        if(size==null || size.length<2)
            return;
        int l=0; // Pointer for 'S'
        int h=size.length-1; // Pointer for 'L'
        int m=0; // Pointer for 'M' Currently evaluating element
        while(m<=h){
            if(size[m]=='S'){
                char temp = size[m];
                size[m] = size[l];
    ...
    public static void  sortTShirtSizeUsingCountingSort(char[] size){
        if(size==null || size.length<2)
            return;
