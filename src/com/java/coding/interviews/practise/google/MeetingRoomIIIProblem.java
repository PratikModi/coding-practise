...
/**Time Complexity: O(m Log n)
    🔹 Complexity
	•	Sorting: O(M log M) where M = number of meetings.
	•	For each meeting: log N (heap ops).
        •	Time: O(M log N + M log M).
        •	Space: O(N + M).
 **/
public class MeetingRoomIIIProblem {
    ...
    public static int mostBooked(int n, int[][] meetings) {
        ...
        // Min-heap of available rooms (store room index)
        PriorityQueue<Integer> freeRoom = new PriorityQueue<>(Integer::compare);
        for(int i=0;i<n;i++){
            freeRoom.add(i);
        }
        ...
        // how many meetings each room handled
        int[] count = new int[n];
        for(int[] meeting : meetings){
            int start = meeting[0];
            int end = meeting[1];
            while(!busyRooms.isEmpty() && busyRooms.peek()[0]<=start){
                freeRoom.add(busyRooms.remove()[1]);
            }
            if(!freeRoom.isEmpty()){
                int roomIndex = freeRoom.poll();
                busyRooms.add(new int[]{end,roomIndex});
                count[roomIndex]++;
            }else{
                //No room is free -> delay in meeting
                int[] earliestEnding = busyRooms.poll(); // earliest finishing room
                int finishTime = earliestEnding[0];
                int roomIndex = earliestEnding[1];
                count[roomIndex]++;
                busyRooms.add(new int[]{finishTime+(end-start),roomIndex});
            }
        }
        int result=0;
        for(int i=1;i<n;i++){
