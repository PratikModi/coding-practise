 ...
 * write pointer writes compressed characters back into the same array (in-place).
 * 2. For each group of consecutive characters
 * Example: "aaabbccccdde"
 * You detect groups:
 * 'a' → count 3
 * 'b' → count 2
 * 'c' → count 4
 * 3. Write compressed form
 * For group 'a' with count 3:
 * ⏱️ Time & Space Complexity
 * Time: O(n) — each char is read and written once.
...
public class StringCompressionProblem {
    ...
    public static int compress(char[] chars) {
        int read=0; // pointer to traverse characters
